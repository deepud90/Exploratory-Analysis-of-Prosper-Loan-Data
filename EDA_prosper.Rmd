...
```{r}
ggplot(data=loans,aes(y=(..count..)*100/sum(..count..),x=ProsperRating..Alpha.))+geom_bar() + scale_y_continuous()+ylab("Percentage of loans")
```
...
p1 = ggplot(data=loans,aes(x=LoanOriginalAmount))+geom_histogram(binwidth=1000,center=500,color='blue')+scale_x_continuous(breaks=seq(0,30000,by=5000),limits=c(0,40000))+ theme(axis.text.x=element_text(angle=45))
...
```{r}
p1 = ggplot(data=loans,aes(x=IncomeRange))+geom_bar(color='blue') + coord_flip()
...
loans$BorrowerStateName[loans$BorrowerState=="DC"]="Washington DC" 
...
-Stats on credit lines

```{r}
p1 = boxplot(loans$CurrentCreditLines)
p2 = boxplot(loans$openCreditLines)
grid.arrange(p1,p2)
```
